...
class DelayDetailsChoiceControllerSpec extends SpecBase with MockUserAnswersService {
  ...
  class Fixture(val userAnswers: Option[UserAnswers] = Some(emptyUserAnswers)) {
    ...
    lazy val view = application.injector.instanceOf[DelayDetailsChoiceView]
  }

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new DelayDetailsChoiceFormProvider()
  val form = formProvider()

  lazy val delayDetailsChoiceRoute = routes.DelayDetailsChoiceController.onPageLoad(testErn, testArc, NormalMode).url

  "DelayDetailsChoice Controller" - {

    "must return OK and the correct view for a GET" in new Fixture() {
      running(application) {

        val request = FakeRequest(GET, delayDetailsChoiceRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in new Fixture(Some(
      emptyUserAnswers.set(DelayDetailsChoicePage, true)
    )) {
      running(application) {
    ...
    "must redirect to the next page when valid data is submitted" in new Fixture() {
      running(application) {
        ...
        val request = FakeRequest(POST, delayDetailsChoiceRoute).withFormUrlEncodedBody(("value", "true"))
        val result = route(application, request).value
    ...
    "must return a Bad Request and errors when invalid data is submitted" in new Fixture() {
      running(application) {
        ...
        val request = FakeRequest(POST, delayDetailsChoiceRoute).withFormUrlEncodedBody(("value", ""))
